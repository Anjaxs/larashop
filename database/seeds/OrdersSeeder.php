    ...
    public function run()
    {
        ...
        foreach ($orders as $order) {
            ...
            $items = OrderItem::factory()->count(random_int(1, 3))->create([
                'order_id'    => $order->id,
                'rating'      => $order->reviewed ? random_int(1, 5) : 0,  // 随机评分 1 - 5
                'review'      => $order->reviewed ? $faker->sentence : '',
                'reviewed_at' => $order->reviewed ? $faker->dateTimeBetween($order->paid_at) : config('app.null_time'), // 评价时间不能早于支付时间
            ]);
